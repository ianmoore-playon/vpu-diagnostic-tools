...
$helpSections = @(
    ...
    @{ H="History tab";                   B="Shows all past diagnostic runs from the Pulse_Results folder. Green rows are All Clear; red rows show Issues Found with the affected port(s). Double-click any row to open the full report in Notepad.`n`nIf the same port appears as Issues Found across many runs, the trend line on the Overview summary card will note this - useful evidence when requesting a replacement." }
    @{ H="Escalating to support";         B="After a run, click Copy Summary in the Actions section. This generates a structured paragraph with port status, SmartSpeed count, camera results, and recommended next steps - paste it directly into your ticket or support chat.`n`nThe Run ID in the summary matches the filename in Pulse_Results so the agent can ask you to email the full report if needed." }
