...
function _suAttestationText(d) {
  ...
  // Win32_ComputerSystem.Model usually already leads with the manufacturer
  // ("HP Z2 Tower G9 ..."), so concatenating both prints "HP HP Z2 ...".
  ...
  if (defOff) {
    ...
    L.push("       they are NOT a lapsed definition update.");
    if (post.level === "current") {
      L.push("       Operating-system security patching (above) is the control that");
      L.push("       applies here, and it is up to date.");
    } else {
...
function renderSoftwareUpdates() {
  ...
  $page().innerHTML = `
    ...
    <div class="card">
      ...
      <p class="text-sm text-pulse-muted mb-3">
        A plain-text summary of everything above, ready to paste into an email or ticket for a
        district IT department asking for proof that patches were applied to this VPU.
      </p>
