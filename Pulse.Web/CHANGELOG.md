...
- Camera Connectivity now shows live PoE power draw for each camera port — watts, volts and amps per port plus the card's total budget, free headroom and temperature, refreshing every few seconds. It warns when the card has little power headroom left, so you know before adding a camera that won't power up. Only the newer camera cards (Intel I210/I211) can report power; on an older 82574L card the section says so plainly instead of sitting empty.
