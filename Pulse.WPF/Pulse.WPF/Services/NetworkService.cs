...
namespace Pulse.WPF.Services
{
    ...
    public class NetworkService : INetworkService
    {
        ...
        {
            // Kick off probes in parallel. Each returns a fully-populated row.
            var tasks = PortTests.Select(RunOnePortTestAsync).ToArray();
        ...
        private static Task<bool> TestUdpNtpAsync(string server)
        {
            return Task.Run(() =>
            {
                UdpClient udp = null;
                try
                {
                    ...
                    var pkt = new byte[48]; pkt[0] = 0x1B;
                    udp = new UdpClient();
                    udp.Client.ReceiveTimeout = TimeoutMs;
                    udp.Client.SendTimeout = TimeoutMs;
