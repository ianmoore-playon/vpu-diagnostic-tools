@echo off
REM ============================================================================
REM  Pulse - share-once launcher (production)
REM
REM  Hand THIS file to field techs (one time). It is intentionally tiny and
REM  never needs to change: on every run it downloads the CURRENT production
REM  launcher from the repo, then runs it. So when we fix or update the
REM  launcher, techs pick it up automatically on their next launch - nobody has
REM  to re-download anything. Keep it on the Desktop and double-click to start
REM  Pulse.
REM ============================================================================
setlocal EnableExtensions
title Pulse - starting

set "LAUNCHER_URL=https://raw.githubusercontent.com/playon/pulse/main/runners/run_pulse.bat"
set "CACHE_DIR=%LOCALAPPDATA%\PulseLauncher"
set "LAUNCHER=%CACHE_DIR%\run_pulse.bat"
if not exist "%CACHE_DIR%" mkdir "%CACHE_DIR%" >nul 2>&1

echo.
echo   Getting the latest Pulse launcher...

REM Download to a temp name first, then swap in - a failed or partial download
REM must never clobber a known-good cached copy.
set "OK="
where curl.exe >nul 2>&1
if not errorlevel 1 (
    curl.exe -fsSL --connect-timeout 10 -o "%LAUNCHER%.new" "%LAUNCHER_URL%" && set "OK=1"
)
if not defined OK (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "try{[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -UseBasicParsing -TimeoutSec 15 -Uri '%LAUNCHER_URL%' -OutFile '%LAUNCHER%.new'; exit 0}catch{exit 1}" && set "OK=1"
)

REM Only accept the download if it looks like a real launcher (>200 bytes),
REM not a tiny error page. Otherwise keep whatever we already had cached.
if defined OK (
    for %%A in ("%LAUNCHER%.new") do if %%~zA GTR 200 move /y "%LAUNCHER%.new" "%LAUNCHER%" >nul
)
if exist "%LAUNCHER%.new" del "%LAUNCHER%.new" >nul 2>&1

if not exist "%LAUNCHER%" (
    echo.
    echo   [ERROR] Couldn't download the Pulse launcher and there's no cached
    echo           copy yet. Check this VPU's internet connection and try again.
    echo.
    pause
    exit /b 1
)

if not defined OK echo   (offline - using the last downloaded launcher)

REM Hand off to the real launcher. It self-elevates (UAC), installs/updates
REM Pulse under C:\Pulse, and opens it in Chrome. %* forwards any arguments.
endlocal & call "%LAUNCHER%" %*
