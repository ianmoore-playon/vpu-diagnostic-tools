...
::
::  - Installs to C:\Pulse
::  - Pulls the latest web-dev-v* pre-release from pulse-releases
::    (falls back to the source repo, then a dev-branch commit zip)
...
:: -- Resolve the latest dev release (tag^|url) ----------------------------
::   1) newest web-dev-v* pre-release on pulse-releases
::   2) same on the source repo
::   3) fallback: dev-branch commit zip, tagged dev-<sha7>
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='SilentlyContinue';[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12;$pat='web-dev-v*';$out='';foreach($repo in @('%PUBLIC_REPO%','%REPO%')){try{$r=Invoke-RestMethod -Uri ('https://api.github.com/repos/'+$repo+'/releases') -TimeoutSec 10}catch{continue};$rel=$r|Where-Object{$_.tag_name -like $pat -and $_.prerelease}|Select-Object -First 1;if($rel){$out=$rel.tag_name+'|'+$rel.assets[0].browser_download_url;break}};if(-not $out){try{$sha=(Invoke-RestMethod -Uri 'https://api.github.com/repos/%REPO%/commits/dev' -TimeoutSec 10).sha;if($sha){$out='dev-'+$sha.Substring(0,7)+'|https://github.com/%REPO%/archive/'+$sha+'.zip'}}catch{}};Write-Output $out" > "%RESOLVE_OUT%" 2>nul
...
if not defined ASSET_URL (
    echo   Update ......................... no dev build found
    goto :dl_failed
)
...
echo   [ERROR] No dev build could be downloaded and none is installed.
